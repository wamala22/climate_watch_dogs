		...
		*/
			...
			thisLayerID, thisLayerType, thisLayerUrl, thisLayerColor, json_url)
		{
			...
			else
			{
				...
				myPoint.transform(proj_4326, mapzzz.getProjectionObject());
			...
			if (mapLoad == 0)
			{
				mapzzz.setCenter(myPoint, myZoom, false, false);
			}
			mapLoad = mapLoad+1;

			// Get Viewport Boundaries
			extent = mapzzz.getExtent().transform(mapzzz.getProjectionObject(),
			...
			markers = mapzzz.getLayersByName(thisLayer);
			if (markers && markers.length > 0)
			{
				for (var i = 0; i < markers.length; i++)
				{
					mapzzz.removeLayer(markers[i]);
				}
			}
