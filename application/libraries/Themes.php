...
function runScheduler(img){img.onload = null;img.src = '" . url::site() . 'scheduler' . "';}
    ...
    public function submit_btn() {
        ...
        if (Kohana::config('settings.allow_reports')) {
            ...
            $btn .= "<form method='post' action='./members/login' id='userpass_form'>";
            $btn .= "<input type='hidden' name='action' value='signin'>";
            $btn .= "<table cellspacing='3' cellpadding='4' id='ushahidi_loginbox'>";
            $btn .= "<tr><td><table><tr>";
            $btn .= "<td><strong>Username:</strong><input type='text' name='username' id='username' class='login_text' /></td>";
            $btn .= "</tr><tr>";
            $btn .= "<td><strong>Password:</strong><input name='password' type='password' class='login_text' id='password' size='20' /></td>";
           // $btn .= "</tr><tr><td><input type='checkbox' id='remember' name='remember' value=1 checked='checked' />Stay logged in on this computer?</td>";
            $btn .= "</tr></table></td>";
            $btn .= "<td><input type='submit' id='submit' name='submit' value='log in' class='searchbtn' />";
            //$btn .= "<a href=\"" . url::site() . "members/login" . "\">Log In</a>";
            $btn .= "</td>";
            //$btn .= "</tr><tr>";
           // $btn .= "<td><a href=javascript:toggle('signin_forgot');> Forgot your password?</a></td>";
            $btn .= "</tr>";
            $btn .= "</table>";
            $btn .= "</form>";
            $btn .= "</div>";
