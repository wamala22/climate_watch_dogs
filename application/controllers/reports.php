...
class Reports_Controller extends Main_Controller {
    ...
    public function view($id = FALSE) {
        ...
        if (!Incident_Model::is_valid_incident($id, TRUE)) {
            ...
            if ($_POST AND Kohana::config('settings.allow_comments')) {
                ...
                if ($post->validate()) {
                    ...
                    $send = notifications::notify_admins(
                                    ...
                                    Kohana::lang('notifications.admin_new_comment.subject'), Kohana::lang('notifications.admin_new_comment.message')
                                    . "\n\n'" . strtoupper($incident->incident_NoOfTrees) . "'"
            ...
            $incident_remarks = nl2br($incident->incident_remarks);
            Event::run('ushahidi_filter.report_title', $incident_NoOfTrees);
            Event::run('ushahidi_filter.report_description', $incident_remarks);
            ...
            $this->template->content->incident_NoOfTrees = $incident_NoOfTrees;
            $this->template->content->incident_remarks = $incident_remarks;
