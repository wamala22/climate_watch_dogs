	...
	public function edit($id = FALSE, $saved = FALSE)
	{
		...
		if ( isset($_GET['mid']) AND intval($_GET['mid']) > 0 ) {
			...
			if ($message->loaded AND $message->message_type == 1)
			{
				...
				$form['incident_remarks'] = $incident_description;
				...
				$form['person_fullName'] = $message->reporter->reporter_first;
				$form['person_contacts'] = $message->reporter->reporter_last;
		...
		if ( isset($_GET['fid']) AND intval($_GET['fid']) > 0 )
		{
			...
			if ($feed_item->loaded)
			{
				...
				$form['incident_remarks'] = $feed_item->item_description;
			...
			if (Incident_Model::is_valid_incident($id))
			{
				...
				if ($incident->loaded == true)
				{
					...
					(
						...
						'incident_remarks' => $incident->incident_remarks,
						...
						'person_fullName' => $incident->incident_person->person_fullName,
						'person_contacts' => $incident->incident_person->person_contacts,
	...
	*/
	...
	function translate( $id = false, $saved = false )
	{
		...
		if ($_POST)
		{
			...
			$post->add_rules('incident_remarks','required');
			...
			if ($post->validate())
			{
				...
				$incident_l->incident_remarks = $post->incident_remarks;
				$incident_l->save();
		...
		else
		{
			if ( $id )
			{
				...
				if ($incident_l->loaded == true)
				{
					...
					$form['incident_remarks'] = $incident_l->incident_remarks;
				}
				else
				{
					// Redirect
		...
		if (is_array($keywords) && !empty($keywords))
		{
			...
			foreach($keywords as $value)
			{
				if (!in_array($value,$stop_words) && !empty($value))
				{
					...
					$where_string = $where_string.$or." incident_remarks LIKE '%$chunk%'  OR location_name LIKE '%$chunk%'";
					$i++;
				}
			}
		}
