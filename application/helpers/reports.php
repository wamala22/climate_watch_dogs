...
class reports_Core {
	...
	public static function validate(array & $post, $admin_section = FALSE)
	{
		...
		if ( ! empty($post->person_fullName))
		{
			$post->add_rules('person_fullName', 'length[3,100]');
		}

		if ( ! empty($post->person_contacts))
		{
			$post->add_rules('person_contacts', 'length[3,200]');
		}
	...
	public static function save_report($post, $incident, $location_id)
	{
		...
		$incident->incident_NoOfTrees = $post->incident_NoOfTrees;
		$incident->incident_remarks = $post->incident_remarks;
	...
	public static function save_personal_info($post, $incident)
	{
		...
		$person->person_fullName = $post->person_fullName;
		$person->person_contacts = $post->person_contacts;
