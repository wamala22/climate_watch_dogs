...
?>
		...
		// Change to 1 after mapzzz loads
		var mapLoad = 0;
		// /json or /json/cluster depending on if clustering is on
		var default_json_url = "<?php echo $json_url ?>";
		// Current json_url, if mapzzz is switched dynamically between json and json_cluster
		...
		*/
		...
		function onFeatureSelect(event)
		{
			...
			{
				mapzzz.removePopup(event.feature.popup);
			}
		...
		// *** Causes the mapzzz to load json twice on the first go
		// *** Need to fix this!
		function mapZoom(event)
		{
			...
			if (mapLoad > 0)
			{
				...
				// Get Current Zoom
				currZoom = mapzzz.getZoom();

				// Get Current Center
				currCenter = mapzzz.getCenter();
		...
		function mapMove(event)
		{
			...
			if (mapLoad > 0)
			{
				...
				if (kmlOverlays.length > 0)
				{
					...
					{
						mapzzz.removeLayer(kmlOverlays[i]);
					}
				}
						...
						if (kmlOverlays.length > 0)
						{
							for (var i = 0; i < kmlOverlays.length; i++)
							{
								...
								if (i == kmlOverlays.length -1)
								{
									...
									mapzzz.addControl(selectControl);
		...
		function showCheckins()
		{
			$(document).ready(function(){
				...
				mapzzz.addControl(highlightCtrl);
		...
		function zoomToSelectedFeature(lon, lat, zoomfactor)
		{
			...
			// Center and Zoom
			mapzzz.setCenter(lonlat, newZoom);
			// Remove Popups
			for (var i=0; i<?php echo '<'; ?>mapzzz.popups.length; ++i)
			{
				mapzzz.removePopup(mapzzz.popups[i]);
			}
		}
		...
		*/
		function externalZeroIn(lon, lat, newZoom, cipopup)
		{
			...
			point.transform(proj_4326, mapzzz.getProjectionObject());
			// Center and Zoom
			mapzzz.setCenter(point, newZoom);
		...
		*/
		function switchLayer(layerID, layerURL, layerColor)
		{
			...
			{
				new_layer = mapzzz.getLayersByName("Layer_"+layerID);
				if (new_layer)
				{
					for (var i = 0; i <?php echo '<'; ?> new_layer.length; i++)
					{
						mapzzz.removeLayer(new_layer[i]);
					}
		...
		jQuery(function() {
			...
			*/
			...
			mapzzz = new OpenLayers.Map('mapzzz', options);
			mapzzz.addControl( new OpenLayers.Control.LoadingPanel({minSize: new OpenLayers.Size(573, 366)}) );
			
			<?php echo map::layers_js(FALSE); ?>
			mapzzz.addLayers(<?php echo map::layers_array(FALSE); ?>);
			
			
			// Add Controls
			mapzzz.addControl(new OpenLayers.Control.Navigation());
			mapzzz.addControl(new OpenLayers.Control.Attribution());
			mapzzz.addControl(new OpenLayers.Control.PanZoomBar());
			mapzzz.addControl(new OpenLayers.Control.MousePosition(
				{
					div: document.getElementById('mapMousePosition'),
					numdigits: 5
				}));    
			mapzzz.addControl(new OpenLayers.Control.Scale('mapScale'));
			mapzzz.addControl(new OpenLayers.Control.ScaleLine());
			mapzzz.addControl(new OpenLayers.Control.LayerSwitcher());
			
			// display the mapzzz projection
			document.getElementById('mapProjection').innerHTML = mapzzz.projection;
			...
			$("a[id^='share_']").click(function()
			{
				...
				{
					share_layer = mapzzz.getLayersByName("Share_"+shareID);
					if (share_layer)
					{
						for (var i = 0; i <?php echo '<'; ?> share_layer.length; i++)
						{
							mapzzz.removeLayer(share_layer[i]);
						}
					}
			...
			if (!$("#startDate").val())
			{
				mapzzz.setCenter(new OpenLayers.LonLat(<?php echo $longitude ?>, <?php echo $latitude ?>), 5);
				return;
			}
