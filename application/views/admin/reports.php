...
<div class="bg">
    ...
    <div class="tabs">
        <!-- tabset -->
        <ul class="tabset">
            <li><a href="?status=0" <?php if ($status != 'a' AND $status != 'v')
            echo "class=\"active\""; ?>><?php echo Kohana::lang('ui_main.show_all'); ?></a></li>
            <li><a href="?status=a" <?php if ($status == 'a')
            echo "class=\"active\""; ?>><?php echo Kohana::lang('ui_main.awaiting_approval'); ?></a></li>
            <li><a href="?status=v" <?php if ($status == 'v')
            echo "class=\"active\""; ?>><?php echo Kohana::lang('ui_main.awaiting_verification'); ?></a></li>
        </ul>
        <!-- tab -->
        <div class="tab">
            <ul>
...
<?php echo $pagination; ?>
            ...
            <tbody>
                ...
                foreach ($incidents as $incident) {
                    ...
                    $incident_NoOfTrees = strip_tags($incident->incident_NoOfTrees);
                    $incident_remarks = text::limit_chars(strip_tags($incident->incident_remarks), 150, "...", true);
                    ...
                    if ($incident_mode == 1) { // Submitted via WEB
                        ...
                        if ($incident->incident_person->id) {
                            ...
                            $submit_by = $incident->incident_person->person_fullName;
                        } else {
                    ...
                    <tr>
                        ...
                        <td class="col-2">
                            <div class="post">
                                <h4><a href="<?php echo url::site() . 'admin/reports/edit/' . $incident_id; ?>" class="more"><?php echo $incident_category; ?></a><?php echo $incident_NoOfTrees; ?></h4>
                                <p><?php echo $incident_remarks; ?>... <a href="<?php echo url::base() . 'admin/reports/edit/' . $incident_id; ?>" class="more"><?php echo Kohana::lang('ui_main.more'); ?></a></p>
                            </div>
                            <ul class="info">
                                <li class="none-separator"><?php echo Kohana::lang('ui_main.location'); ?>: <strong><?php echo $incident_location; ?></strong>,<strong><?php if ($country_id != 0) {
                    echo $countries[$country_id];
                } else {
                    echo $countries[Kohana::config('settings.default_country')];
                } ?></strong></li>
    ...
    ?>
                        ...
                        <td class="col-4">
                            <ul>
                                <li class="none-separator"><a href="#"<?php if ($incident_approved)
        echo " class=\"status_yes\"" ?> onclick="reportAction('a','APPROVE', '<?php echo $incident_id; ?>');"><?php echo Kohana::lang('ui_main.approve'); ?></a></li>
                                <li><a href="#"<?php if ($incident_verified)
                            echo " class=\"status_yes\"" ?> onclick="reportAction('v','VERIFY', '<?php echo $incident_id; ?>');"><?php echo Kohana::lang('ui_main.verify'); ?></a></li>
